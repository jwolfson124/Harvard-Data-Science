...
def get_fpr(y_true, y_pred_proba, threshold):
    ...
    fpr_list.append(fpr)
    return fpr
    ##this loop goes through the thresholds for each knn

### edTest(test_tpr) ###

def get_tpr(y_true, y_pred_proba, threshold):
    # your code here
        #create a varibale to hold the fpr
        #link the values that are going to be used to calculate the fpr to each variable
        #y_true = y_test
        #y_pred_proba = y_pred_knn
    
    y_pred = []
        #this loop goes through each predicted positive probabilities and checks how compares to threshold
